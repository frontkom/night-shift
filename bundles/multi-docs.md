...
NS_BUNDLE=docs
echo "Night Shift docs bundle starting (multi-repo)..."
```
...
Recap of what to do here, no prose: use `NS_RUN_START_EPOCH`, `NS_BUNDLE=docs`, and the `NS_PROCESSED` array you've been filling. A repo is "processed" when its summary row's status is `ok`, `silent`, or `failed`. Best-effort — never let the dashboard append fail the routine.
