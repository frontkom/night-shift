...
- Always bump `NIGHT_SHIFT_VERSION` in `skills/night-shift/SKILL.md` on **any** framework-level change — not just edits to `SKILL.md` itself. That includes changes to `bundles/`, `tasks/`, `manifest.yml`, or anything else under the repo root that a downstream user of the skill would want a refresh signal for. The version is the single fleet-wide "anything new?" marker; users only re-read the skill (and thus discover updated bundle behavior) when it bumps. Increment the letter suffix (e.g. `2026-04-12a` → `2026-04-12b`), or use today's date with suffix `a` if the date changed. The version appears in **two places** that must stay in sync:
